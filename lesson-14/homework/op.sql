...
select left(Name,CHARINDEX(',',Name,1)-1) Name,RIGHT(Name, LEN(Name) - CHARINDEX(',', Name)) Surname from TestMultipleColumns

select * from TestPercent
where Strs like '%!%%' escape '!'

select substring(Vals,1,CHARINDEX('.',Vals,1)-1),
SUBSTRING(Vals,CHARINDEX('.',Vals,1)+1,CHARINDEX('.',Vals,CHARINDEX('.',Vals,1)+1) - CHARINDEX('.',Vals,1)),
SUBSTRING(Vals,CHARINDEX('.',Vals,CHARINDEX('.',Vals,1)+1) +1,len(Vals)-CHARINDEX('.',VAls,CHARINDEX('.',Vals,1)+1)) from Splitter
...
select e.Id, e.Name, e.Salary, m.Name AS ManagerName, m.Salary AS ManagerSalary from Employee e
join Employee m on e.ManagerId=m.Id
where e.Salary>m.Salary

SELECT e.Id, e.Name, e.Salary, m.Name AS ManagerName, m.Salary AS ManagerSalary
FROM Employee e
JOIN Employee m
ON e.ManagerId = m.Id
WHERE e.Salary > m.Salary;
...
from string_split('apple,banana,kiwi,grape', ',')
where ordinal = 3;

select substring('sdgfhsdgfhs@121313131', number, 1) as char_val
...
select cast(left(Vals, patindex('%[^0-9]%', Vals+'a')-1) as int) as StartInteger
from GetIntegers;
...
sum(sum(sales)) over (partition by week) as decimal(10,2)) as pct_sales
from sales
group by week, area;
