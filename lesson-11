...
join Employees E on E.DepartmentID=E.DepartmentID
...
select c.FirstName, count(o.OrderID) TotalOrders
from Customers c
join Orders o
on c.CustomerID = o.CustomerID
...
select c.FirstName, max(o.OrderDate) MostRecentOrderDate from Customers c
left join Orders o on c.CustomerID = o.CustomerID
group by c.FirstName;

select c.FirstName, o.TotalAmount OrderTotal from Customers c
...
select P.ProductName,sum(S.SaleAmount) from Products P
join Sales S on P.ProductID=S.ProductID
...
select P.ProductName,S.SaleAmount,P.Category from Products P
join Sales S on P.ProductID=S.ProductID
where P.Category='Electronics ' or S.SaleAmount>350

select C.CategoryName,count(P.ProductID) from Products P
...
join Departments D on E.DepartmentID=D.DepartmentID
where D.DepartmentName in('HR','Finance') or (LEN(e.Name)
