...
select Category,count(ProductName) from Products P1
group by Category
...
select max(TotalAmount) maxQ,min(TotalAmount) minQ from Orders
...
select avg(TotalAmount), OrderDate from Orders
group by OrderDate
Order by OrderDate desc

select ProductName,case
when Price < 100 then 'low'
when Price between 100 and 500 then 'Midle'
else 'Higth'
End PriceGroup
from Products
...
select max(TotalAmount),CustomerID from Invoices
group by CustomerID
having CustomerID <= 3
...
select count(S.SaleID), P.ProductName from Products P
