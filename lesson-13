...
select Year1,(select max(v) from (values (Max1), (Max2), (Max3)) ValueList(v)) HighestValue
from TestMax;
...
select coalesce(col1, col2, col3, col4) FirstNonNull
from person

select *from Students

select substring(FullName,1,CHARINDEX(' ',FullName,1)-1) as FirstName,
SUBSTRING(FullName,CHARINDEX(' ',FullName,1)+1,CHARINDEX(' ',FullName,CHARINDEX(' ',FullName,1)+1) - CHARINDEX(' ',FullName,1)) as MidleName,
SUBSTRING(FullName,CHARINDEX(' ',FullName,CHARINDEX(' ',FullName,1)+1) +1,len(FullName)-CHARINDEX(' ',FullName,CHARINDEX(' ',FullName,1)+1)) as LastName from Students
...
STRING_AGG(String, ', ') WITHIN GROUP (ORDER BY SequenceNumber) AS concatenated_values
FROM DMLTable
...
sum(case when datediff(year, hire_date, getdate()) > 3 then 1 else 0 end) employees_over_3yrs,
cast(100.0 * sum(case when datediff(year, hire_date, getdate()) > 3 then 1 else 0 end)
/ COUNT() as decimal(5,2)) as percentage_over_3yrs
from employees
...
spaceman_id,
hire_date,
ROW_NUMBER() OVER (PARTITION BY job_description ORDER BY hire_date ASC) AS rn_min,
ROW_NUMBER() OVER (PARTITION BY job_description ORDER BY hire_date DESC) AS rn_max
FROM Personal
)
SELECT job_description,
MAX(CASE WHEN rn_min = 1 THEN spaceman_id END) AS most_experienced,
MAX(CASE WHEN rn_max = 1 THEN spaceman_id END) AS least_experienced
FROM Ranked
GROUP BY job_description;

WITH chars AS (
SELECT value AS ch
FROM STRING_SPLIT('tf56sd#%OqH', '', 1) -- split into single characters (SQL Server 2022+)
