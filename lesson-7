...
select Category,min(Price) minP, max(Price) maxP from Products
group by Category
...
select sum(SaleAmount), avg(SaleAmount) from Sales

select DepartmentName, min(Salary) minS, max(Salary) maxS from Employees
group by DepartmentName

select DepartmentName,avg(Salary), count(EmployeeID) from Employees
group by DepartmentName

select Category, avg(Price) avgP from Products
group by Category
...
select year(OrderDate), min(OrderDate),max(OrderDate)from Orders
