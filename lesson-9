...
select P.PaymentID,O.OrderID from Payments P
...
select O.OrderID,P.StockQuantity,O.Quantity from Orders O
cross join Products P
...
select S.Name,C.CourseName from Courses C
inner join Students S on S.StudentID=C.CourseID
...
select S.ProductID,S.SaleID,C.FirstName,C.City from Sales S
inner join Customers C on C.CustomerID=S.CustomerID
where C.City ='USA'

select O.OrderID,C.FirstName,C.City,O.TotalAmount from Orders O
inner join Customers C on C.CustomerID=O.CustomerID
where C.City ='Germany' and O.TotalAmount>100

select E1.Name Employee1, E2.Name Employee2
from Employees E1
...
select distinct C.FirstName, Orders.OrderID
FROM Orders
inner join Customers C on Orders.CustomerID = C.CustomerID
left join Payments P on Orders.OrderID = P.OrderID
